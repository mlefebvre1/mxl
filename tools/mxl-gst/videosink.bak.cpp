...
namespace
{
    ...
    private:
        ...
        /// Indicates the MXL timestamp at the moment the pipeline got started. Is used to calculate the PTS of the buffers based on the
        ...
        /// We use the caps to initialize the audio metadata when pushing audio samples. There are probably more efficient ways to do this, but
        /// this should be good enough for the first iteration.
        ...
        std::optional<std::string> const& audioFlowID)
    {
        ...
        if (audioFlowID)
        {
            ...
            audio_config = GstreamerAudioPipelineConfig{
                .rate = audio_descriptor_parser.getGrainRate(), .channelCount = audio_descriptor_parser.getChannelCount(), .spkrEnabled = {0, 1}
            };
        }
        else
        {
    ...
    int real_main(int argc, char** argv, void*)
    {
        ...
        while (!g_exit_requested)
        {
            ...
            // Currently there is neither blocking read for audio nor other way how to make sure both audio and video data are available. Here we
            // just hackishly assume that audio is available earlier than video, and thus it is safe to read as much audio as we already have video.
            if (audioReader)
            {
                ...
                {
                    MXL_INFO("Pushing audio samples!");
                    gst_pipeline.pushAudioSamples(audioPayload, firstAudioSampleTimestamp, lastAudioSampleTimestamp, mxlGetTime());
                }
            }
